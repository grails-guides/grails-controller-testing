...
`RestResponse` object and using that object we can validate the returned items of the controller. The first test
...
opposed to the unit test which mocks it out. Also note `serverPort` property is automatically injected.
...
<1> `.json` can be included in the URL to declare we want a JSON response.
