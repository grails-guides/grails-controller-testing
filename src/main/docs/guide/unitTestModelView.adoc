...
`Test the index action returns the correct model` spock test to validate the correct model and count are returned.
...
method appropriately. Also notice in the Spec how we can manipulate the `contentType`, the `request` method with POST, and access the
`redirectedUrl` property in the `response`:
...
----
include::{sourceDir}/src/test/groovy/grails/controller/testing/StudentControllerSpec.groovy[tag=specBeginning]
include::{sourceDir}/src/test/groovy/grails/controller/testing/StudentControllerSpec.groovy[tag=populateParams]
include::{sourceDir}/src/test/groovy/grails/controller/testing/StudentControllerSpec.groovy[tag=testIndex]
include::{sourceDir}/src/test/groovy/grails/controller/testing/StudentControllerSpec.groovy[tag=testSave]
include::{sourceDir}/src/test/groovy/grails/controller/testing/StudentControllerSpec.groovy[tag=specEnding]
----

<3> Since the save method is restricted to POST, we must specify the request.method is a POST call